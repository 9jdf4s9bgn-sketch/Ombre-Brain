...
DEEPSEEK_V4_ATTRIBUTION_BOUNDARY_RULES = """事实与视角边界（强制）：
- 对话里的说法不自动等于客观事实。涉及感受、看法、相信、担心、猜测、理解、反事实或可能性时，content 必须明确保留说话者和主观限定（例如“某人认为/担心/以为/感到”），不得删除限定后改写成事实。
- 如果后文否认、纠正或反驳了前述命题，禁止把被否认或纠正的命题写成事实。若该信息值得保留，必须同时写清“谁持有什么主观判断”以及“谁随后如何否认或纠正”。
- 输出每条 content 前，在内部核对其中每个事实主张是否有原文明示、归属于谁、后文是否否认。不足以确定时，改写为带归属的主观状态或省略；禁止补成双方共同结论，也禁止从一次互动泛化出长期稳定的关系属性。"""
